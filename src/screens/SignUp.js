...
const SignUp = ({ navigation }) => {
  ...
  const [showcnfpassFocus, setcnfshowfocus] = useState(false);
  const [cnfpassFocus, setcnfpassfocus] = useState(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.inputout}>
        <AntDesign name="lock" size={24} color={cnfpassFocus ? colors.text1 : colors.text2} />
        <TextInput style={styles.input}
          placeholder='Confirm Password' onFocus={() => {
            setEmailfocus(false)
            setpassfocus(false)
            setcnfpassfocus(true);
          }}
          secureTextEntry={!showcnfpassFocus ? true : false}
        >
        </TextInput>

        <Octicons name={showcnfpassFocus === false ? "eye-closed" : "eye"} size={24} color="black" onPress={() => {
          setcnfshowfocus(!showcnfpassFocus);
        }} />
      </View>
      ...
      <TouchableOpacity style={btn1}>
        <Text>
          <Text style={{ color: colors.col1, fontSize: title.btntxt, fontWeight: "bold" }}>Sign Up</Text>
        </Text>
      </TouchableOpacity>
